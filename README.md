...
Классификатор разделяет Google Tasks на планируемые действия, наблюдаемые
привычки и полностью игнорируемые записи. В календарь автоматически попадают
только планируемые действия; тренировки, сон и финансовые показатели остаются
для наблюдения, а неактуальные записи о курении не показываются и не
участвуют в предложениях.
...
сразу после ближайшей ночной синхронизации. Дата за три дня до сеанса используется
для сортировки приоритетов и не запрещает начать подготовку раньше. Ассистент
предлагает свободный двухчасовой блок; запись в календарь выполняется после
выбора предложения владельцем. Если в текущем дне места нет, Clippy ищет
ближайшее свободное окно вперёд до рекомендуемой даты подготовки. После добавления
отметки в название или описание события действие исчезает при следующей
синхронизации.
